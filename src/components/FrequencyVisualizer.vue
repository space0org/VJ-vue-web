...
const drawCircularFrequency = (canvasCtx, dataArray, width, height) => {
  ...
  canvasCtx.arc(centerX, centerY, maxRadius * 0.7, 0, Math.PI * 2)
  canvasCtx.stroke()
  
  canvasCtx.beginPath()
  canvasCtx.arc(centerX, centerY, maxRadius * 0.9, 0, Math.PI * 2)
  canvasCtx.stroke()
  ...
  for (let i = 0; i < dataArray.length; i += 8) {
    ...
    const innerRadius = maxRadius * 0.7
    const outerRadius = innerRadius + (amplitude * maxRadius * 0.3)
